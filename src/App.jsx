...
const STORAGE_KEY = "her-rhythm-dynamic-v4";
const ONBOARDING_KEY = "her-rhythm-onboarding-v4";
...
const phaseMeta = {
  ...
  Follicular: {
    ...
    focus: "Use rising energy for training consistency, lighter appetite control, and habit building.",
  ...
  Luteal: {
    ...
    focus: "Support appetite with protein, fiber, regular meals, and lower stress load.",
...
const contraceptionOptions = ["None", "Condom", "Birth control pill", "Patch", "Injection", "IUD", "Implant", "Emergency contraception"];
const activityOptions = ["Walking", "Workout", "Yoga", "Pilates", "Running", "Swimming", "Rest day"];
...
const expectedSymptomsByCategory = {
  ...
  Ovulation: {
    Ovulation: ["Watery cervical mucus", "Clear stretchy mucus", "Egg-white mucus", "Increased libido", "More energy"],
...
const symptomGroups = [
  { title: "Premenstrual", items: ["Cramps", "Pelvic heaviness", "Lower back pain", "Bloating", "Breast tenderness", "Swollen breasts", "Acne breakouts", "Food cravings", "Increased appetite", "Fatigue", "Insomnia", "Poor sleep", "Irritability", "Anxiety", "Mood swings", "Crying spells", "Headache", "Migraine", "Constipation", "Diarrhea", "Spotting"] },
  { title: "Ovulation", items: ["One-sided pelvic pain", "Mild cramping", "Increased libido", "More energy", "Breast sensitivity", "Light spotting", "Wet sensation", "Slippery sensation", "Watery cervical mucus", "Clear stretchy mucus", "Egg-white mucus", "Cervix feels higher", "Cervix feels softer", "Cervix feels more open"] },
  { title: "Post-menstrual", items: ["Dry days", "Very little discharge", "Sticky discharge", "Mild energy rebound", "Reduced bloating", "Fewer cramps", "Stable mood", "Clearer skin", "Lower appetite swings"] },
  { title: "Positive pregnancy", items: ["Missed period", "Nausea", "Vomiting", "Food aversions", "Food cravings", "Increased smell sensitivity", "Taste changes", "Breast pain", "Breast fullness", "Fatigue", "Frequent urination", "Heartburn", "Appetite changes", "Mild cramping", "Milky discharge", "Pregnancy glow"] },
...
const fastingPlans = {
  basic: [
    { title: "12:12", fasting: "12h fasting", eating: "12h eating", pro: false },
    { title: "13:11", fasting: "13h fasting", eating: "11h eating", pro: false },
    { title: "14:10", fasting: "14h fasting", eating: "10h eating", pro: false, selected: true },
    { title: "15:9", fasting: "15h fasting", eating: "9h eating", pro: true },
  ],
  intermediate: [
    { title: "16:8", fasting: "16h fasting", eating: "8h eating", pro: false },
    { title: "17:7", fasting: "17h fasting", eating: "7h eating", pro: true },
    { title: "18:6", fasting: "18h fasting", eating: "6h eating", pro: true },
    { title: "19:5", fasting: "19h fasting", eating: "5h eating", pro: true },
  ],
  advanced: [
    { title: "20:4", fasting: "20h fasting", eating: "4h eating", pro: false },
    { title: "21:3", fasting: "21h fasting", eating: "3h eating", pro: true },
    { title: "22:2", fasting: "22h fasting", eating: "2h eating", pro: true },
    { title: "23:1", fasting: "23h fasting", eating: "1h eating", pro: true },
  ],
};
...
function inferPhaseFromSymptoms(symptoms, cyclePhaseHint) {
  const scores = { Menstrual: 0, Follicular: 0, Ovulation: 0, Luteal: 0, Pregnancy: 0 };
  ...
  const confidence = score === 0 ? 0 : Math.min(97, Math.round((score / Math.max(1, symptoms.length || 1)) * 100) + Math.min(20, (score - second) * 6));
  return {
    phase,
    label: score === 0 ? `Likely ${cyclePhaseHint.toLowerCase()} phase` : `Likely ${phase.toLowerCase()} phase`,
    confidence,
    confidenceLabel: confidence >= 82 ? "Strong match" : confidence >= 58 ? "Moderate match" : confidence > 0 ? "Early signal" : "No clear pattern",
    fertilityLabel: phase === "Ovulation" ? "Likely fertile window" : phase === "Luteal" ? "Likely post-ovulation" : phase === "Follicular" ? "Likely pre-ovulation" : "Likely menstrual timing",
    scores,
  };
}
...
export default function CycleWellnessPage() {
  ...
  useEffect(() => {
    localStorage.setItem(
      ONBOARDING_KEY,
      JSON.stringify({ cycleLength, periodLength, goals, completed: onboardingComplete })
    );
  ...
  useEffect(() => {
    ...
    if (currentCalories !== estimatedCaloriesFromSteps) {
      updateSingleField("activityCalories", estimatedCaloriesFromSteps);
    }
  }, [selectedLog.steps]);
  ...
  const statsData = {
    ...
    ovulation: selectedCycleDay > ovulationDay ? "Past predicted ovulation" : selectedCycleDay === ovulationDay ? "Predicted ovulation" : "Ovulation approaching",
  ...
  const fastingCountdown = useMemo(() => {
    const plan = selectedLog.fastingPlan || "14:10";
    const [fastingHours] = plan.split(":").map(Number);
    const start = selectedLog.fastingStartTime || "18:30";
    const end = selectedLog.fastingEndTime || "08:30";
    ...
    return {
      ...
      fastingHours,
      elapsed: Math.max(0, fastingHours - Math.floor(diff / (1000 * 60 * 60))),
    };
  }, [selectedLog.fastingPlan, selectedLog.fastingStartTime, selectedLog.fastingEndTime, selectedLog.steps]);

  const bmi = useMemo(() => {
    const meters = Number(selectedLog.height || 165) / 100;
    return meters > 0 ? (Number(selectedLog.weight || 65) / (meters * meters)).toFixed(1) : "0.0";
          ...
          <div className="w-full rounded-[2rem] border border-white/10 bg-gradient-to-br from-[#34243a] via-[#241d30] to-[#1a1824] p-6 shadow-2xl">
            {onboardingStep === 1 && (
              <div>
                ...
                <h1 className="mt-5 text-4xl font-bold leading-tight">Understand Your Body, Beyond the Cycle</h1>
                <p className="mt-4 text-base leading-7 text-white/70">
                  Get personalized insights, guidance, and patterns based on your unique rhythm.
                </p>
                <button onClick={() => setOnboardingStep(2)} className="mt-8 w-full rounded-2xl bg-[#d3ae91] px-5 py-4 text-base font-semibold text-[#2b1e1b] shadow-lg shadow-[#d3ae91]/20">
                  Get started
                </button>
              </div>
            )}
...
function Card({ children }) {
  return <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-xl">{children}</section>;
}
...
function NumberField({ label, value, onChange, min, max, step = 1 }) {
  return (
    <label className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <input type="number" min={min} max={max} step={step} value={value} onChange={(e) => onChange(Number(e.target.value) || 0)} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-white outline-none" />
    </label>
  );
}
...
function EntrySection({ title, subtitle, items, groupedItems, expectedItemsByCategory, showAllToggle, showAllSymptoms, onToggleShowAll, selectedItems = [], onToggle }) {
  return (
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {showAllToggle && (
          <div className="flex gap-2">
            <button className="rounded-full bg-[#d3ae91] px-4 py-2 text-sm font-semibold text-[#2b1e1b]">Expected first</button>
            <button onClick={onToggleShowAll} className="rounded-full border border-white/10 bg-black/20 px-4 py-2 text-sm text-white/85">{showAllSymptoms ? "Show expected" : "Show all"}</button>
          </div>
        )}
      </div>
...
function LinkButton({ href, label }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-center text-sm text-white/85 hover:bg-white/10">
      {label}
    </a>
  );
}
...
function StatProgress({ label, value, progress }) {
  return (
    <div>
      ...
      <div className="h-3 rounded-full bg-white/10">
        <div className="h-full rounded-full bg-gradient-to-r from-pink-300 to-blue-300" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} />
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {plans.map((plan) => {
          ...
          return (
            <button key={plan.title} onClick={() => onSelect?.(plan.title)} className={`rounded-[1.5rem] border p-4 text-left ${selected ? "border-emerald-300/30 bg-emerald-300/10" : "border-white/10 bg-white/5"}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="text-4xl font-bold">{plan.title}</div>
                <div className="flex items-center gap-2">
                  {selected ? <div className="rounded-full bg-emerald-300/20 px-2 py-1 text-[10px] font-semibold text-emerald-100">Selected</div> : null}
                  {plan.pro ? <div className="rounded-full bg-[#f0d6b8] px-3 py-1 text-xs font-semibold text-[#6d4c2c]">PRO</div> : null}
                </div>
              </div>
